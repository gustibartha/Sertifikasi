...
// Pastikan kredensial ini sesuai dengan project yang Anda buka di dashboard
...
export default function App() {
  ...
  const [formData, setFormData] = useState({
    nama: "", nid: "", bidang: "", no_sertifikat: "", tgl_expired: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("BERHASIL! Data masuk ke Cloud.");
      setFormData({ nama: "", nid: "", bidang: "", no_sertifikat: "", tgl_expired: "" });
      fetchData();
    } catch (err) {
      alert(`Gagal: ${err.message}\nTip: Jalankan trik RENAME di SQL Editor Supabase!`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="row justify-content-center">
        <div className="col-md-5">
          <div className="card p-4 shadow-sm border-0">
            <h5 className="fw-bold mb-3 text-primary text-center">INPUT SERTIFIKASI</h5>
            <form onSubmit={handleSubmit}>
              ...
              <button className="btn btn-primary w-100 fw-bold" disabled={loading}>{loading ? "MENYIMPAN..." : "SIMPAN KE CLOUD"}</button>
            </form>
          </div>
        </div>
      </div>
